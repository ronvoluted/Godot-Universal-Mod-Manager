...
func test_get_meta_returns_default_when_key_absent():
	var node := Node.new()
	assert_eq(node.get_meta(&"nonexistent", "fallback"), "fallback")
	node.free()


func test_get_meta_returns_value_when_key_present():
	var node := Node.new()
...
func test_get_meta_default_works_with_various_types():
	var node := Node.new()
...
func test_scene_tree_meta_returns_default_when_unset():
	var tree := get_tree()
	assert_eq(tree.get_meta(&"current_game", ""), "")


func test_scene_tree_meta_roundtrip():
	var tree := get_tree()
