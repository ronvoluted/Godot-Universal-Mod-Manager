...
func test_mod_data_constructor_callable_in_map() -> void:
	...
	var mods: Array = dicts.map(Registry.GameData.ModData.new)
...
func test_game_data_constructor_callable_in_map() -> void:
	...
	var games: Array = dicts.map(Registry.GameData.new)
...
func test_assign_typed_array_with_map_constructor() -> void:
	...
	mods.assign(dicts.map(Registry.GameData.ModData.new))
	assert_eq(mods.size(), 2)
	assert_true(mods[0] is Registry.GameData.ModData)
	assert_true(mods[1] is Registry.GameData.ModData)
...
func test_variant_array_map_with_constructor() -> void:
	...
	var mods: Array[Registry.GameData.ModData] = []
	mods.assign(Array(raw).map(Registry.GameData.ModData.new))
	assert_eq(mods.size(), 2)
...
func test_lambda_and_direct_constructor_produce_same_result() -> void:
	...
	var via_lambda := dicts.map(func(d: Dictionary) -> Registry.GameData.ModData: return Registry.GameData.ModData.new(d))
	var via_direct := dicts.map(Registry.GameData.ModData.new)
