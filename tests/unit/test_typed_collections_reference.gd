...
func test_game_data_installed_mods_are_independent() -> void:
	var game_a := GameData.new({entry_path = "/a", game_path = "/ga", installed_mods = []})
	var game_b := GameData.new({entry_path = "/b", game_path = "/gb", installed_mods = []})

	game_a.installed_mods.append(ModData.new({load_path = "/mods/only_a", active = true}))
...
func test_game_data_mod_mutation_does_not_leak() -> void:
	...
	game_b.installed_mods.append(ModData.new({load_path = "/mods/b1", active = true}))
	game_b.installed_mods.append(ModData.new({load_path = "/mods/b2", active = false}))
...
func test_deserialized_games_have_independent_mod_lists() -> void:
	...
	games.assign(raw_games.map(GameData.new))

	assert_eq(games[0].installed_mods.size(), 1)
	assert_eq(games[1].installed_mods.size(), 0)

	games[1].installed_mods.append(ModData.new({load_path = "/mods/b1", active = true}))
...
func test_round_trip_preserves_array_independence() -> void:
	var game_a := GameData.new({entry_path = "/a", game_path = "/ga", installed_mods = [{load_path = "/mods/a1", active = true}]})
	var game_b := GameData.new({entry_path = "/b", game_path = "/gb", installed_mods = []})
	...
	restored[1].installed_mods.append(ModData.new({load_path = "/mods/new", active = true}))
...
func test_empty_typed_arrays_are_distinct() -> void:
	...
	game_a.installed_mods.append(ModData.new({load_path = "/mods/proof", active = true}))
...
func test_map_constructed_games_are_independent() -> void:
	...
	var games: Array[GameData] = []
	games.assign(configs.map(GameData.new))

	games[0].installed_mods.append(ModData.new({load_path = "/mods/x", active = true}))
	games[2].installed_mods.append(ModData.new({load_path = "/mods/y", active = true}))
	games[2].installed_mods.append(ModData.new({load_path = "/mods/z", active = false}))
